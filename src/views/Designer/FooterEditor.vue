                ...
                <div class="absolute inset-0 px-4 sm:px-6">
                  <div class="h-full" aria-hidden="true">
                    ...
                    <div class="flex flex-col" v-if="editSocialMedia">
                      <div v-for="icon in dataDeepCopy.socialMediaSection" :key="icon.id">
                        <div>
                          ...
                          >
                            <option v-for="icon in socialMediaIcons" :key="icon.name">
                              {{ icon.name }}
                            </option>
                          </select>
                        </div>
                      ...
                      <button @click="save()" class="basis-full btn bg-green-500 font-bold">
                        Save
                      </button>
                    </div>
                    <!-- LinksSections -->
                    <div class="flex flex-col" v-else>
                      <div v-for="column in dataDeepCopy.redirectSection" :key="column.id">
                        <div v-if="column.id === targetId" class="pt-10">
                          ...
                          />
                          <div v-for="link in column.list" :key="link.id" class="mb-5">
                            Link
                            <input
                              v-model="link.text"
                              type="text"
                        ...
                        <!-- LinksSections EDIT -->
                        <div v-if="column.list.find((x) => x.linkId === targetLinkId)">
                          Link
                          <input
                            v-model="column.list.find((x) => x.linkId === targetLinkId).text"
                            type="text"
                            class="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                          />
                          URL
                          <input
                            v-model="column.list.find((x) => x.linkId === targetLinkId).url"
                            type="text"
                            class="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                          />
                          <div>
                      ...
                      <button @click="save()" class="basis-full btn bg-green-500 font-bold mb-3">
                        Save
                      </button>
                      <button
...
export default {
  ...
  methods: {
    ...
    deleteElement() {
      const index = this.dataDeepCopy.redirectSection.findIndex((e) => e.id === this.targetId);
  ...
  created() {
    ...
    if (!this.targetFooterSectionId && !this.editSocialMedia && !this.targetFooterSectionLinkId) {
      this.addColumn();
      ...
      this.targetLinkId = JSON.parse(JSON.stringify(this.targetFooterSectionLinkId));
    }
  },
};
</script>
